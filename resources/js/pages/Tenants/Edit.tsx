...
type EditProps = { open: boolean; onClose: () => void; tenant: Tenant | null };
...
export default function TenantEditModal({ open, onClose, tenant }: EditProps) {
    ...
    useEffect(() => {
        if (tenant) {
            form.setData({ name: tenant.name, company_name: tenant.company_name ?? '', status: tenant.status, type: tenant.type ?? '', email: tenant.email ?? '', phone: tenant.phone ?? '', area: tenant.area ?? '', location: tenant.location ?? '', logo: null, description: tenant.description ?? '' });
            setLogoPreview(tenant.logo_url); // eslint-disable-line react-hooks/set-state-in-effect
...
return;
        ...
        form.post(update.url({ tenant: tenant.id }, { query: { _method: 'PUT' } }), {
            onSuccess: () => {
 toast.success('Data Tenant berhasil diperbarui.'); form.reset(); setLogoPreview(null); onClose(); 
},
        });
    };
                ...
                <div className="rounded-[calc(1.5rem-0.375rem)] bg-background flex flex-col overflow-hidden">
                    <DialogHeader className="p-6 pb-4 border-b shrink-0">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <DialogTitle className="text-lg font-semibold text-foreground">Edit Tenant</DialogTitle>
                                <DialogDescription className="text-sm text-muted-foreground">Update the details for the selected tenant.</DialogDescription>
                            </div>
                        </div>
                    </DialogHeader>
                    <form onSubmit={submit} className="flex-1 flex flex-col overflow-hidden">
                        <div className="flex-1 overflow-y-auto p-6 space-y-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                            ...
                            <div className="grid gap-2">
                                ...
                                <div className="flex items-center gap-6">
                                    ...
                                    <div>
                                        ...
                                        <Button type="button" variant="outline" size="sm" onClick={() => fileRef.current?.click()} className="rounded-full px-4">Change Logo</Button>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid gap-5 sm:grid-cols-2">
                                    {[
                                        { id: 'edit-name', label: 'Name', required: true, placeholder: 'Contact person name', key: 'name' as const },
                                        { id: 'edit-company_name', label: 'Company Name', placeholder: 'Company or business name', key: 'company_name' as const },
                                        { id: 'edit-status', label: 'Status', required: true, key: 'status' as const, type: 'select', options: ['active', 'inactive', 'suspended'] },
                                        { id: 'edit-type', label: 'Type', placeholder: 'e.g. Internal, External, Vendor', key: 'type' as const },
                                    ].map((field) => (
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid gap-5 sm:grid-cols-2">
                                    {[
                                        { id: 'edit-email', label: 'Email', placeholder: 'email@example.com', key: 'email' as const, type: 'email' },
                                        { id: 'edit-phone', label: 'Phone', placeholder: '+62 812 3456 7890', key: 'phone' as const },
                                        { id: 'edit-area', label: 'Area', placeholder: 'Operational zone or region', key: 'area' as const },
                                        { id: 'edit-location', label: 'Location', placeholder: 'Full address', key: 'location' as const },
                                    ].map((field) => (
                                        <div key={field.id} className="grid gap-2">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="rounded-xl border border-border/40 bg-black/[0.015] p-1 dark:bg-white/[0.015]">
                                    <textarea id="edit-description" name="description" rows={4} value={form.data.description} onChange={(e) => form.setData('description', e.target.value)}
                        ...
                        <DialogFooter className="p-6 pt-4 border-t bg-accent/20 shrink-0">
                            ...
                            <Button disabled={form.processing} type="submit" className="group rounded-full bg-primary px-6 text-primary-foreground shadow-lg shadow-primary/25 transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] hover:shadow-xl hover:shadow-primary/30 active:scale-[0.97]">
                                {form.processing ? 'Saving...' : 'Update Tenant'}
